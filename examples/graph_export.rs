...
use spatial_narrative::graph::{NarrativeGraph, EdgeType, DotOptions};
...
fn main() {
    ...
    println!("  - Temporal: {}", graph.edges_of_type(EdgeType::Temporal).len());
    println!("  - Spatial: {}", graph.edges_of_type(EdgeType::Spatial).len());
    println!("  - Thematic: {}", graph.edges_of_type(EdgeType::Thematic).len());
